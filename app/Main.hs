...
createTables conn = do
  ...
  execute_ conn "CREATE TABLE IF NOT EXISTS records (id TEXT PRIMARY KEY, group__id TEXT NOT NULL, title TEXT NOT NULL, amount REAL NOT NULL, by__username TEXT NOT NULL, to__username TEXT, date TEXT NOT NULL, FOREIGN KEY (group__id) REFERENCES groups (id) ON DELETE CASCADE, FOREIGN KEY (by__username) REFERENCES users (username), FOREIGN KEY (to__username) REFERENCES users (username));"
  execute_ conn "CREATE TABLE IF NOT EXISTS record_splits (record__id TEXT, user__username TEXT, share INT NOT NULL CHECK (share >= 0), amount REAL NOT NULL, PRIMARY KEY (record__id, user__username), FOREIGN KEY (record__id) REFERENCES records (id) ON DELETE CASCADE, FOREIGN KEY (user__username) REFERENCES users (username));"
